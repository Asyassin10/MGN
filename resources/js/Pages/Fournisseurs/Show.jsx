    ...
    return (
        ...
        >
            <div className="mb-4 grid gap-4 md:grid-cols-3 xl:grid-cols-6">
                <Card className="summary-card"><CardContent><div className="summary-label">Nom</div><div className="summary-value">{fournisseur.nom}</div></CardContent></Card>
                <Card className="summary-card"><CardContent><div className="summary-label">Ville</div><div className="summary-value">{fournisseur.ville || '-'}</div></CardContent></Card>
                <Card className="summary-card"><CardContent><div className="summary-label">Téléphone</div><div className="summary-value">{fournisseur.telephone || '-'}</div></CardContent></Card>
                <Card className="summary-card"><CardContent><div className="summary-label">Factures reçues aujourd’hui ({fournisseur.today_factures_count})</div><div className="summary-value text-red-700">{money(fournisseur.today_total_factures)}</div></CardContent></Card>
                <Card className="summary-card"><CardContent><div className="summary-label">Paiements donnés aujourd’hui ({fournisseur.today_payments_count})</div><div className="summary-value text-emerald-700">{money(fournisseur.today_total_paye)}</div></CardContent></Card>
                <Card className="summary-card"><CardContent><div className="summary-label">Solde</div><div className="summary-value"><Badge variant={fournisseur.balance > 0 ? 'red' : 'green'}>{money(fournisseur.balance)}</Badge></div></CardContent></Card>
            </div>
