                ...
                <TabsContent value="payments">
                    ...
                    <DataTable
                        columns={[
                            ...
                            { key: 'montant', label: 'Montant DH', render: (row) => money(row.montant) },
                            { key: 'actions', label: 'Actions', render: (row) => <div className="flex flex-wrap gap-2"><a href={route('fournisseurs.releves.payments.pdf', [fournisseur.id, releve.id, row.id])}><Button size="sm" variant="outline"><FileText className="h-4 w-4" />PDF</Button></a><CrudDialog title="Modifier paiement" action={route('fournisseurs.releves.payments.update', [fournisseur.id, releve.id, row.id])} method="patch" fields={paymentFields} defaults={row} trigger={<Button size="sm" variant="outline">Modifier</Button>} /><DeleteButton action={route('fournisseurs.releves.payments.destroy', [fournisseur.id, releve.id, row.id])} title="Supprimer ce paiement ?" /></div> },
                        ]}
                        rows={payments.data}
                        pagination={payments}
                    />
