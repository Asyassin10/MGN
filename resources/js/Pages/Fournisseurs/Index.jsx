...
export default function Index({ fournisseurs, filters }) {
    ...
    const columns = [
        ...
        { key: 'total_factures', label: 'Total Factures', render: (row) => <span className="text-lg font-semibold">{money(row.total_factures)}</span> },
        { key: 'total_paye', label: 'Total Payé', render: (row) => <span className="text-lg font-semibold">{money(row.total_paye)}</span> },
        { key: 'balance', label: 'Balance', render: (row) => <span className={`text-lg font-semibold ${row.balance > 0 ? 'text-red-700' : 'text-green-700'}`}>{money(row.balance)}</span> },
    ...
    return <AppLayout title="Fournisseurs" actions={<Link href={route('fournisseurs.create')}><Button><Plus className="h-4 w-4" />Nouveau</Button></Link>}><div className="mb-4 grid gap-2 md:grid-cols-4"><Input placeholder="Recherche nom" defaultValue={filters.search || ''} onChange={(e) => update('search', e.target.value)} /><Input placeholder="Ville" defaultValue={filters.ville || ''} onChange={(e) => update('ville', e.target.value)} /><Input type="number" placeholder="Solde min" defaultValue={filters.balance_min || ''} onChange={(e) => update('balance_min', e.target.value)} /><Input type="number" placeholder="Solde max" defaultValue={filters.balance_max || ''} onChange={(e) => update('balance_max', e.target.value)} /></div><ExportableDataTable columns={columns} rows={fournisseurs.data} pagination={fournisseurs} exportUrl={route('fournisseurs.index')} exportParams={{ ...filters, export: 1 }} onRowClick={(row) => router.visit(route('fournisseurs.show', row.id))} /></AppLayout>;
}
