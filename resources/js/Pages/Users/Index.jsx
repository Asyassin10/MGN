...
export default function Index({ users }) {
    const toDefaults = (user = {}) => ({
        ...user,
        pin: '',
        ...Object.fromEntries(modules.map((module) => [`module_${module}`, user.modules?.includes(module) || user.role === 'admin'])),
        ...Object.fromEntries(modules.filter((module) => module !== 'dashboard').map((module) => [`delete_${module}`, user.delete_modules?.includes(module) || user.role === 'admin'])),
    });
    const normalize = (items) => items.map((field) => field.name.startsWith('module_') ? { ...field, name: field.name } : field);
    return <AppLayout title="Utilisateurs et permissions" actions={<CrudDialog title="Nouvel utilisateur" action={route('users.store')} fields={normalize(fields)} defaults={toDefaults()} trigger={<Button>Nouvel utilisateur</Button>} />}><DataTable columns={[{ key: 'name', label: 'Nom complet' }, { key: 'role', label: 'Profil', render: (row) => row.role === 'admin' ? 'Administrateur' : 'Accès limité' }, { key: 'modules', label: 'Accès autorisés', render: (row) => row.role === 'admin' ? 'Tous les modules' : row.modules.map((module) => moduleLabels[module]).join(', ') || 'Aucun' }, { key: 'delete_modules', label: 'Suppressions autorisées', render: (row) => row.role === 'admin' ? 'Toutes' : row.delete_modules.map((module) => moduleLabels[module]).join(', ') || 'Aucune' }, { key: 'actions', label: 'Actions', render: (row) => <div className="flex gap-2"><CrudDialog title="Modifier utilisateur" action={route('users.update', row.id)} method="patch" fields={fields} defaults={toDefaults(row)} trigger={<Button size="sm" variant="outline">Modifier</Button>} /><DeleteButton action={route('users.destroy', row.id)} title="Supprimer cet utilisateur ?" /></div> }]} rows={users} pagination={{ links: [] }} /></AppLayout>;
}
