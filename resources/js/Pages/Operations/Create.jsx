        ...
        <AppLayout title={editing ? `Modifier ${operation.reference}` : 'Nouvelle opération'}>
            <Card className="max-w-4xl"><CardContent>
                <form onSubmit={submit} className="grid gap-4">
                    ...
                    {errors.lines || errors.operation ? <div className="text-base text-red-600">{errors.lines || errors.operation}</div> : null}
                    <div className="rounded-md border border-zinc-200">{data.lines.map((item, index) => <div key={`${item.article_id}-${index}`} className="flex flex-col gap-2 border-b border-zinc-100 px-3 py-2 text-base last:border-b-0 sm:flex-row sm:items-center sm:justify-between"><span className="min-w-0 truncate">{articles.find((a) => a.value === String(item.article_id))?.label}</span><div className="flex items-center justify-between gap-3 sm:justify-end"><span>{item.quantity}</span><Button type="button" size="icon" variant="ghost" onClick={() => setData('lines', data.lines.filter((_, i) => i !== index))}><Trash2 className="h-4 w-4" /></Button></div></div>)}</div>
