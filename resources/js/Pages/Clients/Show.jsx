        ...
        <AppLayout title={client.nom} actions={<><CrudDialog title={`Modifier ${client.nom}`} action={route('clients.update', client.id)} method="patch" fields={clientFields} defaults={client} trigger={<Button variant="outline">Modifier</Button>} /><DeleteButton action={route('clients.destroy', client.id)} title={`Supprimer ${client.nom} ?`} message="La suppression sera refusée tant que ce client possède des entrées, paiements ou chèques." /></>}>
            ...
            <Tabs defaultValue="entries">
                ...
                <TabsContent value="payments">
                    ...
                    <DataTable columns={[{ key: 'date_paiement', label: 'Date' }, { key: 'montant', label: 'Montant', render: (r) => money(r.montant) }, { key: 'mode', label: 'Mode' }, { key: 'reference', label: 'Référence' }, { key: 'actions', label: 'Actions', render: (r) => <div className="flex flex-wrap gap-2"><a href={route('clients.payments.pdf', [client.id, r.id])} target="_blank" rel="noopener noreferrer"><Button size="sm" variant="outline"><FileText className="h-4 w-4" />Voir PDF</Button></a><CrudDialog title="Modifier paiement" action={route('clients.payments.update', [client.id, r.id])} method="patch" fields={paymentFields} defaults={r} trigger={<Button size="sm" variant="outline">Modifier</Button>} /><DeleteButton action={route('clients.payments.destroy', [client.id, r.id])} title="Supprimer ce paiement ?" /></div> }]} rows={payments.data} pagination={payments} />
                </TabsContent>
            </Tabs>
        </AppLayout>
    );
