    ...
    return (
        <div className="rounded-md border border-zinc-200 bg-white">
                ...
                <Table>
                    ...
                    <TableBody>
                        {rows?.length ? rows.map((row) => (
                            <TableRow key={row.id} onClick={() => onRowClick?.(row)} className={`${rowClassName?.(row) || ''} ${onRowClick ? 'cursor-pointer' : ''}`}>
                                {columns.map((column) => (
            ...
            <div className="divide-y divide-zinc-100 md:hidden">
                {rows?.length ? rows.map((row) => (
                    <div key={row.id} onClick={() => onRowClick?.(row)} className={`p-4 ${rowClassName?.(row) || ''} ${onRowClick ? 'cursor-pointer active:bg-zinc-50' : ''}`}>
                        <dl className="grid gap-2">
                            {detailColumns.map((column) => (
                                <div key={column.key} className="flex items-start justify-between gap-3 text-base">
                                    <dt className="shrink-0 text-sm font-medium uppercase text-zinc-500">{column.label}</dt>
